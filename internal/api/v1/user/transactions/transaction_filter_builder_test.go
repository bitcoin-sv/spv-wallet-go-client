	...
	tests := map[string]struct {
		...
		"transaction filter: zero values": {
			filter: filter.TransactionFilter{
				Id:              testutils.Ptr(""),
				Hex:             testutils.Ptr(""),
				BlockHash:       testutils.Ptr(""),
				BlockHeight:     testutils.Ptr(uint64(0)),
				Fee:             testutils.Ptr(uint64(0)),
				NumberOfInputs:  testutils.Ptr(uint32(0)),
				NumberOfOutputs: testutils.Ptr(uint32(0)),
				DraftID:         testutils.Ptr(""),
				TotalValue:      testutils.Ptr(uint64(0)),
				Status:          testutils.Ptr(""),
			},
