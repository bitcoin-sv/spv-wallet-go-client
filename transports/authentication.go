...
func getSigningMessage(xPub string, auth *buxmodels.AuthPayload) string {
	Log.Info().Str("authentication", "getSigningMessage").Msgf("%s%s%s%d", xPub, auth.AuthHash, auth.AuthNonce, auth.AuthTime)
...
func setSignatureHeaders(header *http.Header, authData *buxmodels.AuthPayload) ResponseError {
	...
	Log.Info().Str("authentication", "setSignatureHeaders").Msgf("%d", authData.AuthTime)
	// Set the time
